...
export default function SiddhaCatalogClient({ products, initialCategory }: SiddhaCatalogClientProps) {
  ...
  const groupedCategories = useMemo(() => {
    ...
    return SIDDHA_NAV_CATEGORIES.map(cat => {
      const items = sortedProducts.filter(p => {
  ...
  const activeCategoryMeta = SIDDHA_NAV_CATEGORIES.find(c => c.slug === selectedCategory);
      ...
      <section className="bg-[#16382B] text-white pt-7 pb-8 px-4 sm:px-6 lg:px-8 border-b border-[#C29043]/30">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-1.5 text-xs text-[#DFB36C] mb-2.5">
            ...
            <span className="font-semibold text-white">{t('Siddha Medicines', 'சித்த மருந்துகள்')}</span>
            {activeCategoryMeta && (
              <>
                <ChevronRight className="w-3 h-3 text-white/40" />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#E8F1EB]/15 border border-[#C29043]/40 text-xs text-[#DFB36C] font-semibold mb-2">
                ...
                <span>{t('Classical Siddha Pharmacopeia of India (111 SKUs)', 'இந்திய பாரம்பரிய சித்த மருந்தியல் (111 தயாரிப்புகள்)')}</span>
              </div>
              <h1 className="font-serif-brand text-2xl sm:text-3xl font-bold tracking-tight">
                {activeCategoryMeta 
                  ? (language === 'ta' ? activeCategoryMeta.titleTa : activeCategoryMeta.title)
                  : t('Classical Siddha Formulations', 'பாரம்பரிய சித்த மருந்துகள்')}
              </h1>
            </div>

            <div className="flex items-center gap-2 text-xs text-[#DFB36C]">
              <ShieldCheck className="w-4 h-4" />
              <span>{t('SPI Standards • Tirunelveli Manuscript Lineage', 'சித்த பார்மகோபியா நெறிமுறைகள்')}</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <main className="flex-1 min-w-0">
                ...
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder={language === 'ta' 
                      ? `தேடுங்கள் (${filteredProducts.length} சித்த தயாரிப்புகள்)...` 
            ...
            {sortedProducts.length === 0 ? (
              <div className="bg-white rounded-2xl border border-[#16382B]/10 p-12 text-center my-6">
                <Leaf className="w-10 h-10 text-[#8A9B93] mx-auto mb-3" />
                <h3 className="font-serif-brand text-lg font-bold text-[#16382B]">
                  {t('No Siddha formulations matched your filters', 'தேர்ந்தெடுக்கப்பட்ட வடிகட்டிகளில் மருந்துகள் இல்லை')}
                </h3>
